...
ifndef ARDUINO_USER_LIB_PATH
ARDUINO_USER_LIB_PATH = $(HOME)/local-dropbox/github/arduino_hacks/libraries
endif
...
# all the objects!
OBJS            = $(LOCAL_OBJS) $(CORE_OBJS) $(LIB_OBJS) $(LIB_OBJS_C) $(LIB_USER_OBJS) $(LIB_USER_OBJS_C)
...
LIB_SRC_C        = $(wildcard $(patsubst %,%/*.c,$(SYS_LIBS)))
LIB_OBJS         = $(patsubst $(ARDUINO_LIB_PATH)/%.cpp,$(OBJDIR)/libs/%.o,$(LIB_SRC))
LIB_OBJS_C       = $(patsubst $(ARDUINO_LIB_PATH)/%.c,$(OBJDIR)/libs/%.o,$(LIB_SRC_C))

# user arguments
USER_LIBS        = $(patsubst %,$(ARDUINO_USER_LIB_PATH)/%,$(ARDUINO_USER_LIBS))
USER_INCLUDES    = $(patsubst %,-I%,$(USER_LIBS))
USER_OBJS        = $(wildcard $(patsubst %,%/*.o,$(USER_LIBS)))
LIB_USER_SRC     = $(wildcard $(patsubst %,%/*.cpp,$(USER_LIBS)))
LIB_USER_SRC_C   = $(wildcard $(patsubst %,%/*.c,$(USER_LIBS)))
LIB_USER_OBJS    = $(patsubst $(ARDUINO_USER_LIB_PATH)/%.cpp,$(OBJDIR)/libs/%.o,$(LIB_USER_SRC))
LIB_USER_OBJS_C  = $(patsubst $(ARDUINO_USER_LIB_PATH)/%.c,$(OBJDIR)/libs/%.o,$(LIB_USER_SRC_C))
...
$(OBJDIR)/libs/%.o: $(ARDUINO_USER_LIB_PATH)/%.cpp
	mkdir -p $(dir $@)
	$(CC) -c $(CPPFLAGS) $(CFLAGS) $< -o $@

$(OBJDIR)/libs/%.o: $(ARDUINO_USER_LIB_PATH)/%.c
	mkdir -p $(dir $@)
	$(CC) -c $(CPPFLAGS) $(CFLAGS) $< -o $@
...
$(TARGET_ELF): 	$(OBJS)
		$(CC) $(LDFLAGS) -o $@ $(OBJS) $(SYS_OBJS) $(USER_OBJS) -lc
